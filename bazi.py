...
if options.b:
    gans = Gans(year=options.year[0], month=options.month[0], 
                day=options.day[0],  time=options.tim·e[0])
...
scores1={'印枭':0,'劫比':0,'财才':0,'官杀':0,'伤食':0}
map1={'印枭':' ','劫比':' ','财才':'0','官杀':'0','伤食':'0'}
...
value_list=[]
rizhuwuxing=''
for key,value in wuhangs.items():
    key_list.append(key)
    value_list.append(value)
for i in range(5):
    if value_list[i].find(me)>=0:
        get_value_index = value_list.index(value_list[i])
        rizhuwuxing = key_list[get_value_index]

print('='*120)
print('  日主是{}，五行属{} '.format(me,rizhuwuxing))

if  rizhuwuxing=='水':
    map1={'印枭':'金','劫比':'水','伤食':'木','财才':'火','官杀':'土'}
if  rizhuwuxing=='木':
    map1={'印枭':'水','劫比':'木','伤食':'火','财才':'土','官杀':'金'}
if  rizhuwuxing=='火':
    map1={'印枭':'木','劫比':'火','伤食':'土','财才':'金','官杀':'水'}
if  rizhuwuxing=='土':
    map1={'印枭':'火','劫比':'土','伤食':'金','财才':'水','官杀':'木'}
if  rizhuwuxing=='金':
    map1={'印枭':'土','劫比':'金','伤食':'水','财才':'木','官杀':'火'}

#计算天干分数
for item in gans:
    scores[gan5[item]] += 5
    gan_scores[item] += 5
    if ten_deities[me][item]=='印':
        scores1['印枭'] += 40
    if ten_deities[me][item]=='偏印':
        scores1['印枭'] += 40
    if ten_deities[me][item] == '劫':
        scores1['劫比'] += 40
    if ten_deities[me][item] == '比':
        scores1['劫比'] += 40
    if ten_deities[me][item]=='偏财':
        scores1['财才'] += 40
    if ten_deities[me][item]=='财':
        scores1['财才'] += 40
    if ten_deities[me][item]=='官':
        scores1['官杀'] += 40
    if ten_deities[me][item]=='杀':
        scores1['官杀'] += 40
    if ten_deities[me][item]=='食':
        scores1['伤食'] += 40
    if ten_deities[me][item]=='伤':
        scores1['伤食'] += 40


#计算地支分数
for item1 in list(zhis):
    for se,item in enumerate(zhichang[item1]):
        if ten_deities[me][item] == '印':
            scores1['印枭'] +=zhichang[item1][item]
        if ten_deities[me][item] == '偏印':
            scores1['印枭'] +=zhichang[item1][item]
        if ten_deities[me][item] == '劫':
            scores1['劫比'] +=zhichang[item1][item]
        if ten_deities[me][item] == '比':
            scores1['劫比'] += zhichang[item1][item]
        if ten_deities[me][item] == '偏财':
            scores1['财才'] +=zhichang[item1][item]
        if ten_deities[me][item] == '财':
            scores1['财才'] +=zhichang[item1][item]
        if ten_deities[me][item] == '官':
            scores1['官杀'] +=zhichang[item1][item]
        if ten_deities[me][item] == '杀':
            scores1['官杀'] +=zhichang[item1][item]
        if ten_deities[me][item] == '食':
            scores1['伤食'] +=zhichang[item1][item]
        if ten_deities[me][item] == '伤':
            scores1['伤食'] +=zhichang[item1][item]
#计算 月建加权
for item1 in list(zhis.month):
    for se,item in enumerate(zhichang[item1]):
        if ten_deities[me][item] == '印':
            scores1['印枭'] +=zhichang[item1][item]*.5
        if ten_deities[me][item] == '偏印':
            scores1['印枭'] +=zhichang[item1][item]*.5
        if ten_deities[me][item] == '劫':
            scores1['劫比'] +=zhichang[item1][item]*.5
        if ten_deities[me][item] == '比':
            scores1['劫比'] += zhichang[item1][item]*.5
        if ten_deities[me][item] == '偏财':
            scores1['财才'] +=zhichang[item1][item]*.5
        if ten_deities[me][item] == '财':
            scores1['财才'] +=zhichang[item1][item]*.5
        if ten_deities[me][item] == '官':
            scores1['官杀'] +=zhichang[item1][item]*.5
        if ten_deities[me][item] == '杀':
            scores1['官杀'] +=zhichang[item1][item]*.5
        if ten_deities[me][item] == '食':
            scores1['伤食'] +=zhichang[item1][item]*.5
        if ten_deities[me][item] == '伤':
            scores1['伤食'] +=zhichang[item1][item]*.5
print('五行强弱：======={}======='.format(map1))
print('五行强弱：======={}======='.format(scores1))

tongdang = scores1['印枭']+scores1['劫比']
yidang = scores1['官杀']+scores1['伤食']+scores1['财才']
geju=''
if tongdang <80:
    geju ="弱极"
elif tongdang< 180:
...
maxscore = max(scores1,key=scores1.get)

print('='*120)
print('     ==========是以{}为主导的{}格  ======== '.format(maxscore,geju))
print('      =========同党得分： {}     ====== 异党得分：{}    '.format(tongdang,yidang))
...
#    ages = [(round(count/3 + 10*i, 2), int(options.year) + 10*i + count//3) for i in range(12)]
    qishinian = round(count/3)
    ages = [(qishinian + 10*i, int(options.year) + 10*i +qishinian) for i in range(10)]
